...
import com.fast.boot.utils.table.ModelTemplate;
import com.fast.boot.utils.table.bean.DataType;
import com.fast.boot.utils.table.bean.Table;
import com.fast.boot.utils.table.bean.TableColumn;
...
public class TableUtils {
	...
	public static void runToMapper(String url, String username, String password, String pakcageName, String mapperPath, String tableNameLike) {
		...
		try {
			...
			for(Table t:tables) {
				if(isPermission(t.getName(), tableNameLike)) {
					tableToDao(pakcageName, mapperPath, t);
	 ...
	 * @param table
	 */
	private static void tableToDao(String packageName, String mapperPath, Table table) {
		String beanName = table.getBean();
		String modelName = ModelTemplate.dealLine(beanName, true);
		Configuration cfg = new Configuration(Configuration.VERSION_2_3_21);
		cfg.setEncoding(Locale.getDefault(), "UTF-8");
		...
		try {
			...
			Map<String, Object> rootMap = new HashMap<String, Object>();
			rootMap.put("modelName", modelName);
			rootMap.put("pakageName", packageName+".mapper."+mapperPath);
			rootMap.put("pakagePath", packageName+"."+mapperPath+"."+"model");
			String javaPath = ModelTemplate.pkgToPathMaven(packageName+".mapper."+mapperPath);
	...
	private static void tableToMapper(String packageName, String mapperPath, Table table) {
		...
		try {
			...
			rootMap.put("modelName", modelName);
			rootMap.put("pakageName", packageName+".mapper."+mapperPath);
			String javaPath = ModelTemplate.pkgToPathMavenResouce("mapper."+mapperPath);

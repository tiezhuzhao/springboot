package com.fast.boot.utils.table;
...
import com.fast.boot.utils.lang.FilenameUtils;
...
public class ModelTemplate {
	 ...
	 */
	public static String buildNameField(List<TableColumn> propertys) throws UnsupportedEncodingException {
		StringBuffer sb = new StringBuffer();
		for (TableColumn property:propertys) {
			...
			if(!"id".equalsIgnoreCase(column)) {
				...
				sb.append(" "+caption);
				if(notnull) {
					String nonull = URLDecoder.decode("（非空） ", "UTF-8");
					sb.append(nonull);
				}
				sb.append(LINE);
			}
